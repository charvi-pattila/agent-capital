...
### Older backlog — cleared 2026-07-22 (see session below)
All six items shipped except the last, which was always a verification task, not a build — see finding below.
...
**Still open:** `backend/requirements.txt` gained `Pillow` — needs `pip install -r requirements.txt` (inside `venv`) before screenshot testing works on a machine that hasn't had it installed ad hoc during testing. Backend route changes (all of the above) need the usual Ctrl+C + rerun `python backend/server.py` to go live; frontend changes need `npm run build` (already done as of this session).
